...
struct HDR2SDR : public I_Render_Task
{
    struct SettingParams : Check_Render_Task_Completeness<SettingParams>
    {
        struct Shader_Params
        {
            ...
            Universal_Type_Wrapper<int> PqMode = {"PqMode", 0, 0, 1};
            auto GetAllAttr() const { return std::tie(texture_path, PqMode); }
        };
        ...
        Universal_Type_Wrapper<string> vsSrc = {"vert shader source", R"(../src/test_frame/glsl/HANDSOUT/hdr2sdr/hdr2sdr.vs.glsl)"};
        Universal_Type_Wrapper<string> fsSrc = {"frag shader source", R"(../src/test_frame/glsl/HANDSOUT/hdr2sdr/hdr2sdr.fs.glsl)"};
        auto GetAllAttr() const { return std::tie(shader_params, will_autogen_frame_wh, frame_width, frame_height, vsSrc, fsSrc); }
    };
    HDR2SDR(string const &name, string const &vsSrc, string const &fsSrc, CentralController *cc)
        : I_Render_Task(name, vsSrc, fsSrc, cc) {}

    Cache_Group_Wrapper<SettingParams> params = {"HDR2SDR", SettingParams{}};
    // texture obj
